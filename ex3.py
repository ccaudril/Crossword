...
#
def readCrossword():
    file = open("crossword_A_v2.txt", "r") # Open the file
    i = 0
    j = 0
    crossword = []
...
def readDictionary(lenghts):
    file = open("diccionari_A.txt", "r") # Open the file
...
#
#   returns a sorted slots list
#
def sort_by_number_of_intersection(crossword, slots):
    for slot in slots :
        i = slot[1]
        j = slot[2]
        n = slot[3]
...
def main():
    ...
    slots = sort_by_number_of_intersection(crossword,slots)
